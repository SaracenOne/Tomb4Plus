...
int16_t* GLaraShadowframe;
float lara_matrices[180];
float lara_joint_matrices[180];
int32_t LaraNodeAmbient[2];
int32_t bLaraUnderWater;
uint8_t LaraNodeUnderwater[15];
int8_t SkinVertNums[40][12];
int8_t ScratchVertNums[40][12];
int8_t bLaraInWater;

int8_t HairRotScratchVertNums[5][12] = {
...
static int32_t lara_mesh_sweetness_table[15] = { 0, 1, 2, 3, 4, 5, 6, 7, 14, 8, 9, 10, 11, 12, 13 };
static int8_t lara_underwater_skin_sweetness_table[15] = { 0, 2, 3, 0, 5, 6, 7, 9, 10, 11, 12, 13, 14, 8, 0 };

static int8_t NodesToStashToScratch[14][2] = {
	{1, 3},
	{4, 5},
	{6, 7},
	{2, 8},
...
static int8_t NodesToStashFromScratch[15][4] = {
	{0, 1, 2, -1},
	{3, 4, -1, 0},
	{5, 6, -1, 0},
	{7, -1, 0, 0},
...
static uint8_t SkinUseMatrix[14][2] = {
	{255, 255},
	{1, 2},
	{255, 255},
	{255, 255},
...
static int32_t in_joints;

bool LaraHasLookTransparencySetting(int32_t current_level) {
...
void DrawLara(ITEM_INFO* item, bool mirror) {
	...
	int32_t* bone;
	int16_t* rot;
	int32_t top, bottom, left, right, dx, dy, dz, dist, stash, xRot;
	static int32_t a = 255;
	...
	for (int32_t i = 0; i < 14; i++) { //joints
		...
		if (SkinUseMatrix[i][0] >= 255)
			...
			xRot = int32_t(32768.0F / (float)M_PI * acosf(v0.x * v1.x + v0.y * v1.y + v0.z * v1.z));
			phd_RotX(int16_t(xRot >> 1));
			phd_PutPolygons(*meshpp, -1);
			phd_PopMatrix();
		}
