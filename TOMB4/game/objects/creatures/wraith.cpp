...
void TriggerWraithEffect(int32_t x, int32_t y, int32_t z, int16_t vel, int32_t objnum) {
	...
	for (int32_t i = 0; i < 15; i++) {
		...
		*(uint32_t*)&sptr->dR = dcol;
		...
		sptr->Xvel = int16_t((rad * phd_sin(ang)) >> W2V_SHIFT);
		sptr->Yvel = (GetRandomControl() & 0x7F) - 64;
		sptr->Zvel = int16_t((rad * phd_cos(ang)) >> W2V_SHIFT);
		sptr->Friction = 4;
		sptr->Flags = SF_SCALE | SF_DEF | SF_UNUSED2;
		sptr->MaxYvel = 0;
		sptr->Scalar = 3;
...
void WraithControl(int16_t item_number) {
	...
	int32_t x, y, z, dx, dy, dz, dist, oob, h, c;
	int16_t rotY, rotX, speed, room_number;
	...
	rotY = int16_t(phd_atan(dz, dx) - item->pos.y_rot);

	if (abs(dx) > abs(dz))
		rotX = int16_t(phd_atan(abs(dx) + (abs(dz) >> 1), dy));
	else
		rotX = int16_t(phd_atan(abs(dz) + (abs(dx) >> 1), dy));
	...
	if (dist < 28900 && abs(item->pos.y_pos - (target->pos.y_pos - (CLICK_SIZE + HALF_CLICK_SIZE))) < CLICK_SIZE) {
		...
		if (target == lara_item) {
			target->hit_points -= int16_t(dist >> 10);
	...
	for (int32_t i = 0; i < (MAX_WRAITH_DATA - 1); i++) {
		...
		if (item->object_number == WRAITH1) {
			...
			wraith->g = uint8_t((GetRandomControl() & 0x3F) + 16 * i + 16);
			...
			wraith->r = uint8_t((GetRandomControl() & 0x3F) + 8 * i + 16);
			wraith->g = wraith->r;
	...
	wraith->xv = int16_t((item->pos.x_pos - x) << 2);
	wraith->yv = int16_t((item->pos.y_pos - y) << 2);
	wraith->zv = int16_t((item->pos.z_pos - z) << 2);
