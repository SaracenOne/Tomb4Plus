...
void ProjectRopePoints(ROPE_STRUCT* Rope) {
	...
	for (int32_t i = 0; i < MAX_ROPE_SEGMENTS; i++) {
		...
		Rope->Coords[i][0] = int32_t(Output.x * zv + f_centerx);
		Rope->Coords[i][1] = int32_t(Output.y * zv + f_centery);
		Rope->Coords[i][2] = (int32_t)Output.z;
	}
...
void phd_GetMatrixAngles(int32_t* m, int16_t* dest) {
	...
	pitch = (int16_t)phd_atan(phd_sqrt(SQUARE(m[M22]) + SQUARE(m[M02])), m[M12]);

	if (m[M12] >= 0 && pitch > 0 || m[M12] < 0 && pitch < 0)
		pitch = -pitch;

	yaw = (int16_t)phd_atan(m[M22], m[M02]);
	sy = phd_sin(yaw);
	cy = phd_cos(yaw);
	roll = (int16_t)phd_atan(m[M00] * cy - m[M20] * sy, m[M21] * sy - m[M01] * cy);
	dest[0] = pitch;
	dest[1] = yaw;
	dest[2] = roll;
}
...
void AlignLaraToRope(ITEM_INFO* l) {
	...
	PHD_VECTOR v1, v2;
	int16_t* frame;
	int32_t temp[indices_count];
	static int32_t ropematrix[indices_count];
	int32_t i, x, y, z, x1, y1, z1;
	int16_t xyz[3];
	static int16_t ropeangle;
	...
	l->pos.x_pos += int32_t(-112 * mMXPtr[M02]);
	l->pos.y_pos += int32_t(-112 * mMXPtr[M12]);
	l->pos.z_pos += int32_t(-112 * mMXPtr[M22]);
	phd_PopMatrix();
	l->pos.x_rot = xyz[0];
	l->pos.y_rot = xyz[1];
	l->pos.z_rot = xyz[2];
...
void ModelRigid(PHD_VECTOR* pa, PHD_VECTOR* pb, PHD_VECTOR* va, PHD_VECTOR* vb, int32_t rlength) {
	...
	Normalise(&d);
	delta.x = (int64_t)scale * d.x >> (W2V_SHIFT + 2);
	delta.y = (int64_t)scale * d.y >> (W2V_SHIFT + 2);
	delta.z = (int64_t)scale * d.z >> (W2V_SHIFT + 2);
	va->x += delta.x;
	va->y += delta.y;
	va->z += delta.z;
	vb->x -= delta.x;
...
void SetPendulumVelocity(int32_t x, int32_t y, int32_t z) {
	...
	if (2 * (CurrentPendulum.node >> 1) < MAX_ROPE_SEGMENTS) {
		...
		x = (int64_t)scale * x >> (W2V_SHIFT + 2);
		y = (int64_t)scale * y >> (W2V_SHIFT + 2);
		z = (int64_t)scale * z >> (W2V_SHIFT + 2);
	}
...
void CalculateRope(ROPE_STRUCT* Rope) {
	...
	if (lara.RopePtr != -1) {
		...
		for (n = Pendulum->node; n >= 0; n--) {
			Rope->Segment[n].x = Rope->MeshSegment[n - 1].x + ((int64_t)Rope->SegmentLength * dir.x >> (W2V_SHIFT + 2));
			Rope->Segment[n].y = Rope->MeshSegment[n - 1].y + ((int64_t)Rope->SegmentLength * dir.y >> (W2V_SHIFT + 2));
			Rope->Segment[n].z = Rope->MeshSegment[n - 1].z + ((int64_t)Rope->SegmentLength * dir.z >> (W2V_SHIFT + 2));
			Rope->Velocity[n].x = 0;
			Rope->Velocity[n].y = 0;
			Rope->Velocity[n].z = 0;
		}
	...
	if (Rope != &RopeList[lara.RopePtr]) {
		...
		for (n = 2; n < MAX_ROPE_SEGMENTS; n++) {
			Rope->MeshSegment[n].x = Rope->MeshSegment[n - 1].x + ((int64_t)Rope->SegmentLength * Rope->NormalisedSegment[n - 1].x >> (W2V_SHIFT + 2));
			Rope->MeshSegment[n].y = Rope->MeshSegment[n - 1].y + ((int64_t)Rope->SegmentLength * Rope->NormalisedSegment[n - 1].y >> (W2V_SHIFT + 2));
			Rope->MeshSegment[n].z = Rope->MeshSegment[n - 1].z + ((int64_t)Rope->SegmentLength * Rope->NormalisedSegment[n - 1].z >> (W2V_SHIFT + 2));
		}
	} else {
		...
		Rope->MeshSegment[Pendulum->node + 1].x = Rope->Segment[Pendulum->node].x + ((int64_t)Rope->SegmentLength * Rope->NormalisedSegment[Pendulum->node].x >> (W2V_SHIFT + 2));
		Rope->MeshSegment[Pendulum->node + 1].y = Rope->Segment[Pendulum->node].y + ((int64_t)Rope->SegmentLength * Rope->NormalisedSegment[Pendulum->node].y >> (W2V_SHIFT + 2));
		Rope->MeshSegment[Pendulum->node + 1].z = Rope->Segment[Pendulum->node].z + ((int64_t)Rope->SegmentLength * Rope->NormalisedSegment[Pendulum->node].z >> (W2V_SHIFT + 2));

		for (n = Pendulum->node + 1; n < (MAX_ROPE_SEGMENTS - 1); n++) {
			Rope->MeshSegment[n + 1].x = Rope->MeshSegment[n].x + ((int64_t)Rope->SegmentLength * Rope->NormalisedSegment[n].x >> (W2V_SHIFT + 2));
			Rope->MeshSegment[n + 1].y = Rope->MeshSegment[n].y + ((int64_t)Rope->SegmentLength * Rope->NormalisedSegment[n].y >> (W2V_SHIFT + 2));
			Rope->MeshSegment[n + 1].z = Rope->MeshSegment[n].z + ((int64_t)Rope->SegmentLength * Rope->NormalisedSegment[n].z >> (W2V_SHIFT + 2));
		}
