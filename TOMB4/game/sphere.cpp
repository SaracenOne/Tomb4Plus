...
SPHERE Slist[34];
int8_t GotLaraSpheres;

static SPHERE LaraSpheres[15];
static int32_t NumLaraSpheres;
...
int32_t GetSpheres(ITEM_INFO* item, SPHERE* ptr, int32_t WorldSpace) {
	...
	ptr->x = x + (int32_t)mMXPtr[M03];
	ptr->y = y + (int32_t)mMXPtr[M13];
	ptr->z = z + (int32_t)mMXPtr[M23];
	ptr->r = meshp[3];
	ptr++;
	phd_PopMatrix();
...
int32_t TestCollision(ITEM_INFO* item, ITEM_INFO* l) {
	...
	PHD_VECTOR lp;
	uint32_t touch_bits;
	int32_t nItemSpheres, nLaraSpheres, ir, lr;
...
void GetJointAbsPosition(ITEM_INFO* item, PHD_VECTOR* pos, int32_t joint) {
	...
	pos->x = item->pos.x_pos + (int32_t)mMXPtr[M03];
	pos->y = item->pos.y_pos + (int32_t)mMXPtr[M13];
	pos->z = item->pos.z_pos + (int32_t)mMXPtr[M23];
	mMXPtr = mMx;
	mIMptr = mIMx;
	phd_mxptr = iMx;
}
