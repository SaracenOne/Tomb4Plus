...
static int16_t FireBounds[12] = { 0, 0, 0, 0, 0, 0, -1820, 1820, -5460, 5460, -1820, 1820 };
...
void DoFlameTorch() {
	...
	if (lara.LitTorch) {
		...
		if (!(wibble & 7))
			TriggerTorchFlame(int16_t(lara_item - items), 0);
	...
	if (GetCollidedObjects(item, 0, 1, itemlist, meshlist, 0)) {
		...
		if (itemlist[0]) {
			if (!objects[itemlist[0]->object_number].intelligent)
				ObjectCollision(int16_t(itemlist[0] - items), item, &mycoll);
		} else {
			...
			ItemPushLaraStatic(item, (int16_t*)&sinfo->x_minc, &pos, &mycoll);
		}
